    ...
    execute(msg, args) {
        return new Promise((resolve, reject) => {
            ...
            while (!character && x <= args.length) {
                character = args.slice(0, x).join(' ');
                character = characters.get(character) || characters.find(c => c.aliases.includes(character));
                x++;
            }
            ...
            const move = args.slice(x - 1).join(' ').toLowerCase();
